...
def main():
    ...
    rospy.init_node('go_to_table_view', anonymous=True)
    ...
    initial_pose = arm_group.get_current_joint_values()
    initial_pose[0] = 0.46
    initial_pose[1] = -0.21
    initial_pose[2] = -0.23
    initial_pose[3] = -0.70
    initial_pose[4] = -0.41
    initial_pose[5] = 2.09  # 1/6 of a turn
    initial_pose[6] = 1.04

    arm_group.go(initial_pose, wait=True)
    arm_group.stop()
    rospy.sleep(2)
    camera_pose = arm_group.get_current_joint_values()
    ...
    camera_pose[3] = -0.31
    camera_pose[4] = 0.58
    camera_pose[5] = 0.81  # 1/6 of a turn
    camera_pose[6] = 0.87

    arm_group.go(camera_pose, wait=True)
    arm_group.stop()
...
#arm_group.go(joint_pose, wait=True)
...
# hand_group.stop()
#pub1 = rospy.Publisher('/panda/panda_finger1_controller/command', Float64, queue_size=10)
#pub2 = rospy.Publisher('/panda/panda_finger2_controller/command', Float64, queue_size=10)
